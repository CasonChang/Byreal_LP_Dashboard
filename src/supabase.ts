...
/**
 * 寫入/更新「今天」的每日聚合一筆（依台北日期 upsert）。
 * 每輪收集都呼叫：當天那筆會持續被更新，跨日後自然定格成那天的最終值。
 * 一天只有一筆 → 看數月/數年歷史也不會撞到 1000 筆讀取上限。
 * 需先在 Supabase 跑 daily_equity.sql；未建表時只警告、不中斷。
 */
export async function saveDailyEquity(snap: PortfolioSnapshot): Promise<void> {
  const d = db();
  if (!d) return;
  const date = new Date(snap.capturedAt).toLocaleDateString('en-CA', { timeZone: 'Asia/Taipei' });
  const row = {
    date,
    liquidity_usd: snap.totals.liquidityUsd,
    lifetime_fees_usd: snap.strategy?.lifetimeFeesUsd ?? snap.totals.earnedUsd,
    pnl_usd: snap.strategy?.lifetimePnlUsd ?? snap.totals.pnlUsd,
    fee_apr: snap.strategy?.feeApr ?? snap.totals.weightedApr,
    total_apr: snap.strategy?.totalReturnApr ?? 0,
    updated_at: new Date().toISOString(),
  };
  const { error } = await d.from('daily_equity').upsert(row, { onConflict: 'date' });
  if (error) console.warn('寫入 daily_equity 失敗（是否已跑 daily_equity.sql？）:', error.message);
}
...
> {
  ...
  const addDailyFee = (rows: Array<any>) => {
    rows.sort((a, b) => (a.date < b.date ? -1 : 1)); // 由舊到新
  ...
  // ===== 1) 優先讀「每日聚合表」daily_equity：一天一筆，可看數月~數年，不受 1000 筆上限影響 =====
  const sinceDate = new Date(Date.now() - days * 86400_000).toLocaleDateString('en-CA', { timeZone: 'Asia/Taipei' });
  const daily = await d
    .from('daily_equity')
    .select('date,liquidity_usd,lifetime_fees_usd,pnl_usd,fee_apr,total_apr')
    .gte('date', sinceDate)
  ...
  if (!daily.error && daily.data && daily.data.length > 0) {
    return addDailyFee(
      daily.data.map((r: any) => ({
        ...
        lifetimeFeesUsd: Number(r.lifetime_fees_usd ?? 0),
        pnlUsd: Number(r.pnl_usd ?? 0),
        feeApr: Number(r.fee_apr ?? 0),
        totalApr: Number(r.total_apr ?? 0),
        dailyFeeUsd: 0,
      })),
    );
  }

  // ===== 2) 退回：尚未建 daily_equity 時，直接從 raw snapshots 聚合 =====
  // 由新到舊 + 明確 limit，避免撞到 PostgREST 預設 1000 筆上限（一天 ~144 筆，由舊到新+預設1000 會把近期截掉）。
  ...
  const LIMIT = 6000; // 約 40 天 @ 10 分鐘/筆
  const first = await d
    .from('snapshots')
    .select(fullCols)
  ...
  for (const r of data) {
    ...
    if (!byDay.has(day)) byDay.set(day, r); // data 由新到舊 → 第一個遇到的即當天最新一筆
  }
  return addDailyFee(
    [...byDay.entries()].map(([date, r]) => ({
      date,
